...
source_dependencies:
  - Plan 81 SP03 spec (~/.claude-plans/81-claude-stem-dogfood-optimization/03-standards/spec.md §Governance Architecture — Two-Surface Dual Pattern, L174-222)
  ...
  - Live ~/.claude-plans/ENFORCEMENT-MAP.md (canonical R-01..R-56 ledger; Lane J empirical measurements)
  - Live ~/.claude/hooks/pre-write-guard.sh + post-write-verify.sh (runtime gate implementations)
  - Live ~/.claude/hooks/vault-schema.json + doc-dependencies.json (hand-authored Claude-consumed registries; dual-surface PoC)
  - Live ~/Documents/Obsidian Vault/Vault Architecture/Vault Architecture - Frontmatter.md (narrative-spoke PoC)
  - Anthropic Skills progressive disclosure (claude.com/skills documentation)
  - Cursor .cursor/rules/*.mdc multi-file scoped-rules pattern
  - GitHub Copilot path-scoped instructions (.github/instructions/)
  - AGENTS.md nested-instructions convention
  - Plan 81 SP03 D1 resolution 2026-05-11 (folder-lineage frontmatter convention)
  - Plan 81 SP03 D2 resolution 2026-05-11 (system-utility dimension exemption + log-subtype-registry T-34)
...
The pattern reuses the most-converged 2026 industry primitive (multi-file with scope-frontmatter; see §Industry convergence below) without inventing a new vocabulary. Cursor calls them `.cursor/rules/*.mdc`. GitHub Copilot calls them path-scoped instructions. Anthropic Skills use progressive disclosure. The shape is the same: small files, declared scope, lazy loading. Plan 81 lands the shape inside vault governance.

## The R-XX numbering convention

Every enforcement rule in Peter's system carries a stable `R-NN` identifier registered in `~/.claude-plans/ENFORCEMENT-MAP.md`. The convention has four properties:

- **Stable.** Rule IDs never renumber. R-01 introduced in spine-remediation Session 02 is still R-01 in May 2026. Retired rules (R-55 retired 2026-05-11) keep their slot with a `RETIRED` marker; the row stays so historical commits and incident reports stay readable.
- **Append-only.** New rules take the next integer. ENFORCEMENT-MAP currently runs through R-56 with R-55 retired and R-32 reassigned once (Session 14 2026-05-11 collision-resolve documented in-row). No backfill, no reorder.
...
The canonical ENFORCEMENT-MAP.md lives at `~/.claude-plans/ENFORCEMENT-MAP.md` (with a vault-side reading copy at `Reference/ENFORCEMENT-MAP.md`). It is a process artifact — the human-readable narrative ledger of the system's enforcement history. It is NOT a runtime artifact: no hook loads it as data. Hooks reference it only in header comments ("# R-XX rules implemented here") and in their own R-27 whitelist so the ledger's own writes don't trip plan-status enforcement. This distinction matters for the architecture decisions below.

**Anti-pattern: renumbering on insert.** Rules added mid-history get the next sequential ID, not a logical insert point. Renumbering breaks every historical citation. The ENFORCEMENT-MAP ledger is append-only by construction.
...
**PreToolUse hooks** — fire before a tool call executes. Used for write-time validation: schema conformance, path allowlisting, structural rule enforcement. Canonical implementation: `~/.claude/hooks/pre-write-guard.sh`. Outputs DENY (block the call), ALLOW (proceed silently), or WARN (proceed with operator annotation). R-32 Tier 2 DENY (frontmatter validation), R-04 (vault-root allowlist), R-09 (Logs/ deny-list), R-15 (plan-backlog row), R-26 (context-pressure mandate), and ~30 more rules fire through this layer. The hook reads governance JSON registries (`vault-schema.json`, `doc-dependencies.json`, post-Plan-81: the 4 pillar files) at runtime.

**PostToolUse hooks** — fire after a tool call returns. Used for post-write verification, cascade emission, and self-healing nudges. Canonical implementation: `~/.claude/hooks/post-write-verify.sh`. Less DENY-heavy than PreToolUse; mostly emits structured findings to logs and surfaces them at session-close. R-07 cascade audits and R-47 orphan-tag advisories live here.

**SessionStart / SessionEnd hooks** — fire at session boundary. Used for context loading (SessionStart) and reconciliation (SessionEnd). Cross-session-pollution incidents (the 2026-05-10 Plan 84 trigger) live in this layer. SessionStart hooks include `prompt-context.sh` (R-26 mandate injection), `session-register.sh` (per-session checkpoint rotation per Plan 84 SP01), and `cron-health-banner.sh` (R-02). SessionEnd hooks include `auto-commit-surfaces.sh` (R-17) and `reconcile-sessions.sh` (R-42).

**PreCompact hook** — fires before context compaction. Used for state preservation across the compact boundary. `pre-compact-checkpoint.sh` writes the Session Continuity Block schema (R-26) and now (post-Plan-84-SP01) writes to per-session paths only. The 2026-05-10 incident — peer session captured another session's checkpoint via destructive `mv` — fundamentally reshaped this category; pre-Plan-84 single-file checkpoint.md is retired.
...
Plan 81 adds two new capabilities in this pattern. `governance-parity-audit` (T-33) compares each pillar JSON to its narrative spoke field-by-field and emits drift findings by pillar. `packet-staleness-audit` (T-19) walks system-altitude research packets and surfaces those approaching the 180-day `last_reviewed` cadence. `log-subtype-audit` (T-34, per D2) detects unregistered `#log/*` and `#status/*` values across the vault.
...
Governance ships across two surfaces with separate consumers but synchronized content. This is the load-bearing architectural commitment of the packet — every other decision flows from it.

### Surface 1 — Claude-consumed (foundation-repo `~/Code/claude-stem/governance/`)
...
Narrative markdown spokes following Peter's existing 7-spoke pattern. Each spoke is hand-authored, carries Peter's voice + pedagogy, and ships pre-populated in the adopter scaffold (SP04 installs from foundation-repo into adopter's vault).

| File | Size target |
|---|---|
| `Vault Architecture - Frontmatter.md` | 4-8K (ports as-is from Peter's live vault) |
| `Vault Architecture - Tagging.md` | 4-8K (NEW; T-30) |
| `Vault Architecture - Naming.md` | 4-8K (NEW; T-31) |
| `Vault Architecture - Mandatory-Files.md` | 4-8K (NEW; T-32) |
| `Vault Architecture - Enforcement.md` | 3-5K (NEW thin meta-spoke; T-32) |
...
Bounded drift is tolerated (Peter's live system runs at 2-3 types of drift between schema and spoke today; the system functions). Visibility is guaranteed: drift surfaces in the weekly audit, not in incident response three months later.

**Anti-pattern: aligning surfaces by generation.** A "generate the narrative spoke from the JSON registry" approach was considered and rejected. Narrative spokes carry Peter's voice, examples, anti-patterns, and citations — content that doesn't round-trip through JSON without lossy transformation. Generated narrative loses pedagogy. R-37 lockstep + audit catches drift without flattening the spokes.

### Why this architecture (the Lane J empirical evidence)

The architecture was decided by Plan 81 SP03 Session 4 (2026-05-11) after a single-comprehensive-lane research dispatch (Lane J). Three findings shaped it:

**Finding 1 — `librarian-manifest.json` is machine-generated inventory, not governance.** Lane J measured: `Logs/librarian-manifest.json` is 150K with 4,407 lines, fully regenerated on every full scan. The actual hand-authored Claude-consumed governance JSONs are `vault-schema.json` (3.8K, 265 lines) and `doc-dependencies.json` (10K, 233 lines) — both small, both runtime-loaded by hooks. Plan 81's ask is therefore *extending an existing pattern from 2 files to 5*, not introducing a new pattern.

**Finding 2 — ENFORCEMENT-MAP.md is NOT load-bearing for runtime.** Lane J grepped `pre-write-guard.sh` + `post-write-verify.sh` for ENFORCEMENT-MAP references. It appears only in header comments and the R-27 whitelist. No hook loads it as data. ENFORCEMENT-MAP at 90K with 4,247-character rows is a *process artifact* (the R-37 narrative ledger) — purely a human and dispatched-Claude reading cost, 23-28K tokens per read with section-of-interest ratio under 10%.

**Finding 3 — the dual-surface pattern already exists.** `vault-schema.json` (Claude-consumed) and `Vault Architecture - Frontmatter.md` (user-consumed) are the frontmatter pillar already split into two surfaces. They've coexisted four weeks with ~2-3 types of bounded drift. R-37 lockstep holds them aligned. Plan 81 *documents and scales* the existing dual-surface pattern from 1 pillar to 4 — it is not inventing a new architecture.

**Anti-pattern: the monolith.** ENFORCEMENT-MAP.md as the canonical runtime-consumed surface was the original framing (pre-Lane-J). It fails on three independent axes simultaneously: (a) hooks have no runtime load — the file is comments and whitelist references only; (b) 23-28K-token read cost crowds out skill-listing budget per dispatched session; (c) wide-table monolith with 145+ rows fails every 2026 industry-convergence threshold for scoped-rule files. The monolith is preserved as the historical narrative ledger (where it works) and superseded as a runtime artifact (where it never was).
...
The convergence across four independent implementations: (1) multi-file is the dominant pattern past small-monolith size; (2) each file declares its scope via frontmatter; (3) a small always-on overlay carries cross-cutting rules; (4) detailed reference material is load-on-demand; (5) file-size hygiene is a first-class concern with documented thresholds. ENFORCEMENT-MAP at 90K with 4,247-character rows is on the wrong side of every documented threshold.

Plan 81's 4-pillar JSON + 4-spoke narrative + thin `_index.json` + thin meta-spoke maps onto the converged primitive cleanly. The `_index.json` is the always-on overlay; pillar JSONs are scope-declared; narrative spokes are load-on-demand from the user side; sizes are bounded.

## Folder-lineage convention (D1 resolution, 2026-05-11)
...
The convention has a structural enforcement contract: T-20 specifies an R-32 hook contract (folder-lineage validation) that DENIES writes where `engagement:` + `project:` field values don't match the directory ancestor names. Tier assignment is SP01's call — advisory Tier 1 → DENY Tier 2 promotion eligible once empirical drift is measured.

**Decision-traceability.** `engagement` and `project` were retired from the canonical TYPE allowlist in the D1 resolution (zero files at TYPE; 237 + 125 files at FIELD slots in Peter's vault — empirical disposition decided the call). The retirement closes a long-deferred ambiguity: are `engagement` and `project` archetypes, or are they navigation slots? Answer: navigation slots, encoded as fields.
...
## System-utility dimension exemption (D2 resolution, 2026-05-11)

The 25-tag cap discipline (research-backed by Forte / Dubois / cognitive working-memory literature; see `tagging-strategy.md` packet) applies to user-facing dimensions. System-utility dimensions — `#log/*` and `#status/*` — are exempt. They are machine-emitted by skills, crons, and capabilities; they never enter the user's working vocabulary.

The exemption is not "anything goes." System-utility dimensions are subject to a different discipline: the **log-subtype registry** (T-34). Every routine activity must use a STABLE, canonical tag value across runs. Example: every `backlog-hygiene` execution tags `#log/backlog-hygiene` — never `#log/backlog-cleanup`, never `#log/backlog-audit`. New subtypes register via Hook A pattern (the adopter is prompted to register the new subtype + owner; commits to Layer 3 vault-overlay).

The enforcement contract has three layers:

- **Registry primitive** at `~/Code/claude-stem/governance/log-subtype-registry.json` — canonical enumeration of all `#log/*` and `#status/*` subtype values + the skill/cron that owns each.
...
- **Hook gate (R-NEW; SP01 assigns the ID at lockstep)** — when a file is written to `Logs/` with a `#log/*` or `#status/*` tag, `pre-write-guard.sh` consults the registry. Tag matches a registered value → ALLOW. Near-match (Levenshtein ≤2 or substring containment) → DENY with "did you mean #log/<canonical>?" Genuinely new → require registration via Hook A.

**Decision-traceability.** The exemption was driven by empirical measurement: Peter's vault has 46 distinct `#log/*` values, dramatically above the 25-cap. The values are canonical operational subtypes (digest-run, session-close, cron-error, etc.) — they are not noise. The 25-cap was designed for user-facing dimensions; applying it to system-utility dimensions would either retire useful operational granularity or force the cap to be widened in a way that defeats its working-memory rationale. The exemption is the structurally honest answer: different disciplines for different consumers.
...
- **OQ-E1** (deferred to SP04 execution): exact `R-NEW` rule ID for log-subtype near-match DENY behavior — SP01 lockstep ledger assigns at hook implementation.
- **OQ-E2** (deferred to SP05 execution): `governance-parity-audit` finding categories may expand beyond the initial four (`rule-id-mismatch`, `field-missing`, `tier-mismatch`, `source-divergence`) once drift patterns are empirically observed. Schema permits extension.
- **OQ-E3** (deferred to SP04 install.sh contract): two-destination install (`~/.claude/governance/` for JSONs; adopter vault `Vault Architecture/` for spokes) needs an idempotent re-install contract for adopters upgrading between Plan 81 increments. Plan 81 SP09 T-4 surfaced an analogous collision pattern (`feedback_plan_81_83_install_collision`); the resolution informs SP04 design.

## Closed questions (with disposition)

- **CQ-1** Per-pillar split vs unified single file? → **Per-pillar.** Decided Session 4 (Lane J Part 5; Peter approval). Rationale: per-pillar load locality matches hook access pattern; unified forces longer jq selectors + more loaded bytes than needed.
- **CQ-2** Generation pattern vs R-37 lockstep for alignment? → **R-37 lockstep + audit.** Decided Session 4. Rationale: narrative spokes carry Peter's voice + pedagogy that doesn't round-trip through JSON without loss; generation flattens spokes.
- **CQ-3** Anthropic skill-bundle pattern as canonical home (`~/.claude/skills/govern/`)? → **Rejected as canonical; preserved as secondary surface.** Decided Session 4 (Lane J Part 8; +1 unconsidered option). Rationale: distribution is vault-first; canonical narrative must live in vault, not in `~/.claude/skills/` orphaned from vault navigation paradigm.
- **CQ-4** Should ENFORCEMENT-MAP.md retire entirely? → **No — preserved as historical narrative ledger.** Decided Session 4. Rationale: ledger function works at 90K (append-only history); the failure was treating it as a runtime artifact. Process role preserved; runtime role moves to pillar JSONs.

## Source pointers

- Plan 81 SP03 spec §Governance Architecture: `~/.claude-plans/81-claude-stem-dogfood-optimization/03-standards/spec.md` L174-222
- Plan 81 SP03 Session 4 architecture decision: `~/.claude-plans/81-claude-stem-dogfood-optimization/03-standards/Session-04-architecture-decision.md` (Peter-approved 2026-05-11, full Lane J synthesis)
- Plan 81 SP03 Session 4 follow-up (D1, D2 resolutions): `~/.claude-plans/81-claude-stem-dogfood-optimization/03-standards/handoff.md` L1044-1120
- Live ENFORCEMENT-MAP ledger: `~/.claude-plans/ENFORCEMENT-MAP.md` (canonical) / `~/Documents/Obsidian Vault/Reference/ENFORCEMENT-MAP.md` (reading copy)
- Live hooks: `~/.claude/hooks/pre-write-guard.sh`, `~/.claude/hooks/post-write-verify.sh`, `~/.claude/hooks/pre-compact-checkpoint.sh`, `~/.claude/hooks/session-register.sh`
- Live Claude-consumed governance JSONs (dual-surface PoC): `~/.claude/hooks/vault-schema.json`, `~/.claude/hooks/doc-dependencies.json`
- Live narrative spoke (PoC): `~/Documents/Obsidian Vault/Vault Architecture/Vault Architecture - Frontmatter.md`
...
- Plan 84 SP01 cross-session-pollution incident class (informs SessionStart/PreCompact category description): `~/.claude-plans/84-multi-session-hooks-state-integrity/01-per-session-checkpoint-paths/` (closed 2026-05-11)
- Companion T-9 dependencies: T-23..T-28 (governance JSONs), T-29..T-32 (narrative spokes), T-33 (governance-parity-audit contract), T-34 (log-subtype-registry per D2)
