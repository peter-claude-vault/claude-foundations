...
**PostToolUse hooks** — fire after a tool call returns. Used for post-write verification, cascade emission, self-healing nudges, and structural-enforcement bootstrap. Canonical implementation: `hooks/post-write-verify.sh`. Less DENY-heavy than PreToolUse; mostly emits structured findings to logs and surfaces them at session-close. R-07 cascade audits, R-47 orphan-tag advisories, and the `_index.md` live-sync hook (one-line `_index.md` self-exempt loop guard at entry; see [`_index.md-design.md`](./_index.md-design.md) §Maintenance architecture Tier 1) live here.

The post-write hook also fires the **new-folder bootstrap action set** when a write hits a folder lacking expected structural artifacts. Folder creation is not a separate event Claude Code surfaces — folders come into existence as side effects of file writes — so the first-write to a new folder IS the bootstrap trigger. The action set coordinates four cross-packet actions: (1) **auto-create `_index.md`** if the folder is non-exempt and lacks one — owned by [`_index.md-design.md`](./_index.md-design.md) §Maintenance architecture Tier 1 step 2; (2) **propagate folder-lineage frontmatter** to the written file (e.g., `engagement: <X>`, `project: <Y>` derived from path) — owned by [`frontmatter-design.md`](./frontmatter-design.md) folder-lineage convention; (3) **add the new folder's entry to its parent `_index.md`** — recursive call into action 1's same logic against the parent folder; (4) **emit a `bootstrap-auto-created` governance audit log entry** for operator visibility at next session-start. The action set is structural — the universal mandatory-file lock is enforced at first-write, not via advisory finding. Each packet documents the action it owns; this enforcement-map registers the umbrella and the order of operations.
...
- **Categorized findings.** Each finding carries a category so the operator can triage by class, not by line. Findings split into two semantic subclasses: **drift categories** surface problems requiring resolution — examples include `stale-status-no-evidence`, `provides-canonicality-drift`, `cron-log-architecture-mismatch`, `governance-parity-drift`, `index-row-drift`. **Event categories** surface notifications of structural-enforcement actions the hook took on the operator's behalf — `bootstrap-auto-created` (a `_index.md` was auto-created at first-write to a new non-exempt folder per the new-folder bootstrap action set above); informational, no action required.
...
The architecture adds four new capabilities in this pattern. `governance-parity-audit` compares each pillar JSON to its narrative spoke field-by-field and emits drift findings by pillar. `packet-staleness-audit` walks system-altitude research packets and surfaces those approaching the 180-day `last_reviewed` cadence. `log-subtype-audit` detects unregistered `#log/*` and `#status/*` values across the vault (see §System-utility dimension exemption below). `index-maintain` walks every non-exempt folder's `_index.md` and reconciles the contents-enum table against the folder's filesystem reality — adding missing entries, removing orphan rows, correcting drifted line counts and types; see [`_index.md-design.md`](./_index.md-design.md) §Maintenance architecture for the Tier 2 + Tier 3 contract. `index-maintain` is the first self-healing capability in the canonical set under the R-34 boundary — its mutations are bounded to mechanically-derivable values (line count via `wc -l`, type from frontmatter, presence of an entry); semantic content (descriptions, ordering, exemptions) is flagged for review and not auto-overwritten.
...
- Companion narrative packets: [`frontmatter-design.md`](./frontmatter-design.md), [`vault-construction-principles.md`](./vault-construction-principles.md), [`tagging-strategy.md`](./tagging-strategy.md), [`_index.md-design.md`](./_index.md-design.md) (Tier 1 post-write hook + Tier 2/3 `index-maintain` capability registered with this packet)
