...
The model is the reference deployment's validated shape. A live `vault-schema.json` ran this structure through multi-week production validation (dissolved SP13 T-4; content migrated to `governance/frontmatter-rules.json#types`; runtime bundle at `governance/foundation-master.json`); the schema-walker hook at `pre-write-guard.sh` now consumes the bundle via bundle-at-load per SP13 T-3; the narrative spoke at `System Governance - Frontmatter.md` documents it. The foundation-repo ports the proof-of-concept and scales it from one pillar's worth of validation to six (the 6-pillar governance architecture per canonical §A).
...
3. **The narrative spoke** — `System Governance - Frontmatter.md` (rendered from the foundation-repo scaffold)
...
- **R-37 lockstep coupled-surface peers**: `governance/frontmatter-rules.json` (rule-level peer of the schema) + `System Governance - Frontmatter.md` (narrative spoke rendered from the scaffold)
