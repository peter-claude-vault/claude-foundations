...
| PROCESSING | Structure | Applies rules, frontmatter, tags, and routing decisions to incoming + existing content | PreToolUse hooks (R-32 frontmatter validation, R-47 advisory orphan detection), in-session ingestion routing via the `/ingest` skill (three-action sequence per SP05), ENFORCEMENT-MAP rules, librarian capabilities | Standards without enforcement drift. Processing is what keeps the system coherent over time without manual maintenance. |
