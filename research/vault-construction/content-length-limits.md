...
| Topic-altitude packet | Vault: `Reference/<topic>/` | 4–10K | 20K | Single-fetch domain primer | Topic primers are read once when starting work in a domain; budget similar to narrative spokes. 90-day review cadence. |
| Initiative-altitude packet | `{plans_root}/<plan>/00-ideation-brief.md` | 6–12K | 25K | Single-fetch session-start load | Reference-deployment range across sampled plans: ~6K–26K. Larger ideation briefs reflect master-initiative complexity; class baseline is 6–12K for single-scope plans. Cap closes at plan close-out. |
...
| Folder-scoped CLAUDE.md | `<folder>/CLAUDE.md` | 1–3K | 5K | Always-on folder-entry load | Pedagogically dense, small enough to load on every folder-scoped agent invocation. Reference-deployment range: ~3–5K typical, with outliers up to ~9K reflecting engagement complexity. Target captures the typical case; outlier signals candidate split. |
| `_index.md` | `<folder>/_index.md` | 0.5–2K | 3K | Single-fetch folder-contents enumeration | Reference-deployment range across sampled folders: ~0.75K–2.6K; an outlier near ~9K signals a legacy artifact candidate for split. Index files are scannable enumerations, not narrative — kept very small by design. |
