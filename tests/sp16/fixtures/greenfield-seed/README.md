...
Hermetic synthetic content for `tests/sp16/greenfield-end-to-end.sh`. Drives the full `/onboard --seed-content <path>` greenfield pipeline (Stage-1 INGEST → IR-build → resume past Sections A–E → Finalize bootstrap-schemas → Section F seven-surface auto-author + four-stage infer-vault orchestrator) under stub mode.
...
`tests/sp16/_lib/section-f-fixture.sh::emit_synthetic_ir()` is reused by other SP16 tests for orchestrator-only path-coverage assertions, but the IR consumed by the full pipeline here comes from `ir-builder.sh` walking `vault-content/`.
