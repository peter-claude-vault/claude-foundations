...
**Status:** active — adopter-facing reference for the foundation installer.
**Audience:** developers adopting Claude Foundations v2 for the first time.
**Companion:** [adopt.md](adopt.md) for fresh-vault scaffolding (`/adopt`).

---

## What it is

`install.sh` materializes the foundation engine — hooks, skills, schemas, onboarding scripts, installer plumbing, plist templates, and the claude-mem plugin — into a target `$CLAUDE_HOME` directory. It is **CLAUDE_HOME-first**: the directory is named explicitly via env, never inferred from `$HOME/.claude` defaults. Defaults to **dry-run**; `--apply` is required to make any filesystem changes.

The script writes 14 asset categories (hooks, lib → hooks/lib, 8 skills, 6 schemas, full onboarding subtree, orchestrator, installer, templates, claude-mem plugin, foundation-manifest.json), atomically merges a `settings.json` snippet, and emits a deterministic provenance log under `$CLAUDE_HOME/logs/install-<ts>-<pid>.log`.
...
| `--force-install`        | Acknowledge G1-main / G2 sentinel; required when `$CLAUDE_HOME` already contains non-foundation content.    |
| `--force-all`            | Catch-all override across guards (G1-G10). Use sparingly — narrows blast-radius assumptions.                |
| `--no-preserve-config`   | Permit clobbering claude-mem `settings.json` keys. Requires `--force-install` (mutual-exclusion gate).      |
| `--backup-dir <path>`    | Custom backup target for G3 proof-of-life check. Default: `$CLAUDE_HOME/.pre-install-<ts>/`.                |
| `--backup-dir=<path>`    | Same as `--backup-dir <path>` in `=`-form.                                                                   |
| `--retrofit-existing`    | Permit install over a non-empty `$PLANS_HOME` (NN-*/ plans present). Reserved for adopters with prior plans.|

---

## Exit codes

`install.sh` uses a structured exit-code map so adopters can branch programmatically. Codes group by surface: 0 success, 10/11 prereq + write surface, 21 state, 30/40 schema/merge, 51-58 G1-G10 guards, 59 reserved.

| Code | Surface              | Cause                                                                                       |
|------|----------------------|---------------------------------------------------------------------------------------------|
| 0    | success              | Install completed (dry-run JSON emit OR `--apply` round-trip).                              |
| 10   | prereq missing       | `CLAUDE_HOME` unset/empty (G1-pre); required binary absent; SOURCE_REPO not a foundation-repo.|
| 11   | write failure        | Permission denied; G10 provenance-log write failed; `--no-preserve-config` without `--force-install`.|
| 21   | state                | `$CLAUDE_HOME` contains only non-foundation content (state=user-only) without `--force-install`.|
...
| 53   | G3                   | Backup proof-of-life: `--backup-dir` absent OR not writable OR round-trip-broken when destructive op pending.|
| 54   | G4                   | Vault-symlink reachable under `$CLAUDE_HOME` (April-13 protection — no override).           |
...
**v2.1-deferred reserved codes (not yet emitted):**

| Code | Reserved name        | What it will be in v2.1                                                                     |
|------|----------------------|---------------------------------------------------------------------------------------------|
| 20   | conflict-manifest    | Refuse on collision with unrelated `$CLAUDE_HOME` content; layered atop G2 sha256 drift.    |
| 22   | rsync-backup         | `--backup-mode=rsync` failure (alternate backup backend; v2.0 ships cp-R only).             |
| 56   | G6-explicit          | Install-time G6 namespace gate when foundation asset's in-content Label drifts. Symmetric with `bootout-launchd.sh` exit 56.|
...
`install.sh` runs ten ordered guards before any destructive step. Each guard has a single responsibility and emits a specific exit code on refusal. See [april-13-autopsy.md](april-13-autopsy.md) for the historical incident motivating G1/G2/G4.
...
The `I-UNDERSTAND-APRIL-13` sentinel is shared between G1-main and G2: a single ceremony per install invocation. Set the sentinel at the prompt, do not commit it.
...
`cp -R` is the v2.0 backup backend; `--backup-mode=rsync` is reserved for v2.1 (exit 22).
...
**Install refuses with exit 54 (G4).** A symlink under `$CLAUDE_HOME` resolves into a vault tree. This is the April-13 protection. There is no override. Remove the symlink before re-running.
...
- [`april-13-autopsy.md`](april-13-autopsy.md) — historical incident motivating G1/G2/G4.
