...
# === FOUNDATION_TEST_MODE write-safety gate (SP00 T-4, 2026-04-22) =========
# Isolation-harness rollback safety. When FOUNDATION_TEST_MODE=1, this hook
# becomes a narrow allowlist enforcer for the SP00 dogfood cycle and all
# existing R-rule branches are BYPASSED (the mode is designed for harness
# test runs, not vault hygiene).
#
# Default (FOUNDATION_TEST_MODE unset or != 1): branch is a no-op; existing
# enforcement paths run unchanged (purely additive).
#
# Allowlist (ALLOW): path under $DOGFOOD_ROOT, $CLAUDE_HOME, $PLANS_HOME
# (falls back to $PLANS_DIR), /tmp/foundation-test-*, or
# /var/folders/**/foundation-test-*. Anything else: DENY with diagnostic
# naming FOUNDATION_TEST_MODE. Structured JSON log line appended to
# $HOOKS_STATE/foundation-test.log on both outcomes.
#
# First downstream consumer: SP01 T-1 live-env schema migration rollback
# safety (plan 71-claude-foundations-engine-v2, sub-plan 00-isolation-harness).
if [[ "${FOUNDATION_TEST_MODE:-0}" == "1" ]]; then
  ...
  FT_LOG="$HOOKS_STATE/foundation-test.log"
  mkdir -p "$HOOKS_STATE" 2>/dev/null || true
  FT_TS=$(date -u +"%Y-%m-%dT%H:%M:%SZ")
  FT_LINE=$(jq -nc \
    --arg ts "$FT_TS" \
    --arg decision "$FT_DECISION" \
    --arg tool "$TOOL_NAME" \
    --arg file "$FILE_PATH" \
    --arg reason "$FT_REASON" \
    --arg dogfood_root "${DOGFOOD_ROOT:-}" \
    '{ts: $ts, decision: $decision, tool: $tool, file: $file, reason: $reason, dogfood_root: $dogfood_root}' \
    2>/dev/null || true)
  if [[ -n "$FT_LINE" ]]; then
    printf '%s\n' "$FT_LINE" >> "$FT_LOG" 2>/dev/null || true
  fi

  if [[ "$FT_DECISION" == "deny" ]]; then
    jq -n --arg r "$FT_REASON" '{
      hookSpecificOutput: {
    ...
    jq -n --arg r "$FT_REASON" '{
      hookSpecificOutput: {
        ...
        additionalContext: ("[FOUNDATION_TEST_MODE] allow: " + $r)
      }
    }'
  fi
  exit 0
fi
# === end FOUNDATION_TEST_MODE gate =========================================
