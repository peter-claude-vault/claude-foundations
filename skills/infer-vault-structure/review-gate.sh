#!/usr/bin/env bash
# review-gate.sh — User review/edit UX gate over import-plan.md.
#
# Wires the three-step gate library (`onboarding/lib/three-step-gate.sh`)
# around the import-plan.md output. Surfaces 4 user actions at the
# preview prompt:
...
#       gate_apply (single audit stream — reuses the auto-author-log;
#       differentiation comes from surface_id="seed-import-plan" plus
#       the action field)
#   Schema-types:
#     - Input: import-plan.md MUST carry `schema_version: import-plan/1`
#       in YAML frontmatter; review-gate refuses to consume non-conformant
#       plans.
...
#       reassembling the wrapper from the markdown).
...
#
# Constraints: bash 3.2 — no `declare -A`, `mapfile`, or `${var,,}`.
# `jq` REQUIRED on PATH (carried through from gate library).
...
usage() {
  cat <<EOF
review-gate.sh — User review/edit UX gate over the import plan.

Usage:
  review-gate.sh [--import-plan PATH] [--approved-out PATH]
                 [--gate-lib PATH] [--accept-on-eof]
...
Exit codes:
  ...
  1   user abort
  2   pre-flight failure (input missing, gate lib missing, schema_version
      mismatch)
EOF
}
...
# ----- pre-flight: input plan exists + schema_version import-plan/1 -----
...
if [ ! -f "$GATE_LIB" ]; then
  ...
  printf '  Expected onboarding/lib/three-step-gate.sh at this location.\n' >&2
  exit 2
fi
